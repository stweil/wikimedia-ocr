...
$( () => {
	...
	$select2.on( 'select2:unselect', ( e ) => {
		const selectedValue = e.params.data.id;
		selectedLanguages = selectedLanguages.filter( ( value ) => selectedValue !== value );
	} );
	...
	if ( $ocrOutputDiv.length ) {
		...
		const $modeButtons = $form.find( '.drag-mode' );
		...
		const initCropper = () => {
			...
			cropperInstance = new Cropper( img, {
				...
				ready() {
					...
					document.getElementById( 'text' ).style.height = cropperInstance.getContainerData().height + 'px';
					...
					$modeButtons.off( 'click.cropmode' ).on( 'click.cropmode', ( event ) => {
						...
						cropperInstance.setDragMode( $button.data( 'drag-mode' ) );
					} );
					...
					// Rotation buttons
					$form.find( '.rotate-left' ).off( 'click.rotate' ).on( 'click.rotate', ( e ) => {
						e.preventDefault();
						e.stopPropagation();
					...
					$form.find( '.rotate-right' ).off( 'click.rotate' ).on( 'click.rotate', ( e ) => {
						e.preventDefault();
						e.stopPropagation();
						if ( cropperInstance && typeof cropperInstance.rotate === 'function' ) {
							cropperInstance.rotate( 90 );
		...
		$form.find( '[name=image]' ).on( 'input', function () {
			const url = $( this ).val();
			if ( url ) {
				...
				$form.find( '.copy-button' ).addClass( 'hidden' );
